...
<head>
    ...
    <title>ثبت اطلاعات شخص</title>
</head>

<body>
    <?php
    //ersal etlaat shakhsi .
    if(isset($_POST['btn1']))
    {
        require_once 'config.php';
              ...
              $data1=$_POST['frm1'];
              $code=$data1['numper'];
              $family=$data1['familyper'];
              $name=$data1['nameper'];
             if($name !=null && $family !=null && $code !=null ){
  echo $name ." ". $family ." " .$code ;
  $connect=mysqli_connect("127.0.0.1","root","","takhalof");
             $query="insert into person (melli,name,last) values ('$code','$name','$family');";
        ...
        <div>
            <form method="POST"  class="container was-validated rounded p-3 bg-light">
                <h2>ثبت اطلاعت شخص</h2>
                <input type="text" placeholder="نام " class=" form-control w-75 mx-auto " id="pwd1"  name="frm1[nameper]" required>
                <input type="text" placeholder="نام خانوادگی" class=" form-control w-75 mx-auto " id="pwd2"    name="frm1[familyper]" required>
                <input type="text" placeholder="کد ملی" requiredclass=" form-control w-75 mx-auto " id="uname"  name="frm1[numper]">
                <button type="submit" name="btn1" id="btn1" class="  w-50 btn-outline-primary">ثبت </button>
